...
#define GLR_PAGE_PROTECT (GLR_PAGE_EXEC | GLR_PAGE_READ | GLR_PAGE_WRITE)
    ...
    inline int page_flags_protect(int flags);
    inline int page_flags_memory(int flags, int base);
    ...
    GLR_FD glr_mem_alloc(size_t size, int flags) {
        ...
        uint64_t id = glr_atomic_add(&physical_backing_id);
        snprintf(path, sizeof(path), "/glr(%lu,%llu)", getpid(), id);
    ...
    void* glr_mem_map(GLR_FD fd, void* addr, size_t size, int flags) {
        int protect = page_flags_protect(flags);
        int memory = page_flags_memory(addr, flags, MAP_PRIVATE | MAP_ANONYMOUS);
        addr = mmap(addr, size, protect, memory, fd, 0);
